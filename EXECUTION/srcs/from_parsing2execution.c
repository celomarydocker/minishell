...
/*   Updated: 2021/03/31 18:44:58 by mel-omar@st      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_clist	*combine_files_and_redirections(const t_clist *files,
const t_clist *redirections)
{
	t_clist	*combine;
...
static t_exec	*parse_exec(const t_ccommand *command, const char *path)
{
	...
	exec->files = combine_files_and_redirections(command->data->files,
			command->data->oper);
	return (exec);
}
